...
export const CustomSessionControls: React.FC<{
  ...
  const createSession = React.useCallback((name?: string, modelProvider?: string, model?: string) => {
    ...
    // Default to 'google' (not 'openai') since our default model is gemini-2.5-flash
  ...
  // Always create new session on page load and set to gemini-2.5-flash
  ...
  // Set new sessions to always use gemini-2.5-flash (only for newly created sessions, not user selections)
  ...
  React.useEffect(() => {
    if (currentSession && currentSession.id && !processedSessionIds.has(currentSession.id)) {
      if (currentSession.model !== 'gemini-2.5-flash') {
        console.log('🔄 [Model Default] Setting new session to gemini-2.5-flash (session:', currentSession.id, ')');
